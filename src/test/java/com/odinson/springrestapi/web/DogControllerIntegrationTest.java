...
public class DogControllerIntegrationTest {
    ...
    @Test
    public void getDogBreedsById() {
        ResponseEntity<String> response = this.restTemplate.getForEntity("http://localhost:" + port + "/breedsbyid/1", String.class);
        assertThat(response.getStatusCode(), equalTo(HttpStatus.OK));
    }

    @Test
    public void getDogNames() throws Exception {
        ResponseEntity<List> response = this.restTemplate.getForEntity("http://localhost:" + port + "/names/", List.class);
        assertThat(response.getStatusCode(), equalTo(HttpStatus.OK));
    }
}
